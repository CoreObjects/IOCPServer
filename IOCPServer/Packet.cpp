...
CPacket::CPacket(uint32_t command, const uint8_t* data, uint32_t size)
	: packetData(2 + 4 + 4 + size + 4, 0) // header, command, length, content, checksum
{
	constructPacket(command, data, size);
}

CPacket::CPacket(uint32_t command, const std::vector<uint8_t>& content) 
	: packetData(2 + 4 + 4 + content.size() + 4, 0) // header, command, length, content, checksum
{
...
CPacket::CPacket(const char* data) {
	...
	packetData.resize(2 + 4 + 4 + length + 4, 0);
...
CPacket::CPacket(uint32_t command)\
	: packetData(2 + 4 + 4 + 4, 0) // header, command, length, content, checksum
{
